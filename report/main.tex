...
\begin{figure}
  \centering
  \fbox{\includegraphics[scale=0.80, trim=5.1cm 4.3cm 1.5cm 3.7cm, clip]{screenshot_coverage}}
...
\begin{figure}
  \centering
  \fbox{\includegraphics[scale=0.75,trim=0cm 1.4cm 6.7cm 8.6cm, clip]{screenshot_mutation_coverage}}
...
This section does not aim at providing a survey of the field (for which there are already~\cite{jones2007automatic,nazar2016summarizing,neubig2016survey,wang2018comment} \todo{give some info on what is the focus of each survey}), but to present the general techniques used as well as the kind of results those tools produce.
...
\todo{SWUM~\cite{hill2009automatically,hill2010integrating}}
\todo{programmers tend to choose descriptive and meaningful names for code units~\cite{liblit2006cognitive}}
They usually contain well structured phrases that can be used to construct NL descriptions of code units without having to understand the code.

Rastkar et al.~\cite{rastkar2010summarizing} worked on summarizing bug reports.
...
\todo{}
\todo{not particularly interested in it as we want to explain a corner case (i.e.\ a mutant)}
\todo{can use stereotypes to put more confidence that the right focal method was identified}
...
\todo{A unit test case typically has fous phases: setup, exercise, verify and teardown~\cite{meszaros2007xunit}.}
...
Li et al.~\cite{li2016automatically} have written a C\# tool that generates descriptions for test methods.
The output, a GUI window, is comprised of 4 pieces of information:
\begin{enumerate}
  \item a general sentence describing the purpose of the test method (based on class, method and arguments signatures using the SWUM approach);
...
The API-based stereotypes allows to extract fine grained what information --- for example, with \mintline{java}{}\todo{}
On the other hand, data/control flow analyses can capture the context of the API calls.\todo{}
...
Except for Proteum/IM 2.0~\cite{delamaro2001proteum} that uses a case study, all the papers related to human assistance reviewed here were evaluated with developers surveys.
